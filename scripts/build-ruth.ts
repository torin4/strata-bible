// Build content/ruth.ts: the complete book. Four readings, two movements, the composition panel,
// two movement capstones and the book capstone.
//
// One generator for the whole book rather than one per ticket. Ruth is 85 verses across four
// readings, and the other authoring generators in this repo place a NEW reading into a large
// existing file (placeReading refuses an id already present, on purpose). Flipping a grounded
// skeleton to a sitting is a replacement, not a placement, so for a book this size emitting the
// file whole keeps content/ruth.ts reproducible from one command and needs no new shared helper.
//
// Usage: npx tsx scripts/build-ruth.ts
...
console.log(
  "Wrote content/ruth.ts: four sittings, two movements, book capstone.",
);
